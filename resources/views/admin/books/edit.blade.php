...
@section('header_actions')
    <a href="{{ route('admin.books.index') }}"
        class="py-2.5 px-4 bg-white border border-slate-200 text-slate-600 hover:bg-slate-50 hover:text-slate-800 hover:border-slate-300 rounded-lg text-sm font-bold shadow-sm hover:shadow transition-all active:scale-95 flex items-center gap-2">
    ...
    <div class="bg-white dark:bg-[#1e293b] rounded-2xl shadow-sm border border-slate-100 dark:border-slate-800 p-6 md:p-8">
        <form action="{{ route('admin.books.update', $book) }}" method="POST" enctype="multipart/form-data"
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                {{-- Title --}}
                <div class="space-y-1">
                    <label for="title" class="block text-sm font-semibold text-slate-700 ml-1">Book Title <span
                            class="text-red-500">*</span></label>
                    <input type="text" name="title" id="title" value="{{ old('title', $book->title) }}" required
                        class="w-full bg-slate-50 border border-slate-200 text-slate-800 text-sm rounded-lg focus:ring-2 focus:ring-primary/20 focus:border-primary block p-3 outline-none transition-all">
                    @error ('title') <span class="text-xs text-red-500 ml-1">{{ $message }}</span> @enderror
                </div>

                {{-- Slug --}}
                <div class="space-y-1">
                    <label for="slug" class="block text-sm font-semibold text-slate-700 ml-1">Slug <span
                            class="text-red-500">*</span></label>
            ...
            <div class="space-y-1">
                <label for="description" class="block text-sm font-semibold text-slate-700 ml-1">Description <span
                        class="text-red-500">*</span></label>

                <div
                    class="rich-editor border border-slate-200 rounded-lg overflow-hidden bg-white hover:border-slate-300 focus-within:ring-2 focus-within:ring-primary/20 focus-within:border-primary transition-all">
                    <!-- Toolbar -->
                    <div class="flex items-center gap-1 p-2 border-b border-slate-100 bg-slate-50/50">
                        <button type="button" data-command="bold"
                            class="p-1.5 text-slate-500 hover:text-slate-800 hover:bg-slate-200 rounded transition-colors"
                            title="Bold">
                    ...
                    <!-- Editable Area -->
                    <div class="editor-content p-4 min-h-[150px] max-h-[400px] outline-none text-slate-800 text-sm overflow-y-auto prose prose-sm max-w-none"
                        contenteditable="true" placeholder="Write a comprehensive description..."></div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                {{-- Price --}}
                <div class="space-y-1">
                    <label for="price" class="block text-sm font-semibold text-slate-700 ml-1">Price (USD)</label>
                    <input type="number" name="price" id="price" value="{{ old('price', $book->price) }}" step="0.01"
                        class="w-full bg-slate-50 border border-slate-200 text-slate-800 text-sm rounded-lg focus:ring-2 focus:ring-primary/20 focus:border-primary block p-3 outline-none transition-all">
                    @error ('price') <span class="text-xs text-red-500 ml-1">{{ $message }}</span> @enderror
                </div>

                {{-- Gumroad URL --}}
                <div class="space-y-1 md:col-span-3">
                    <label for="gumroad_url" class="block text-sm font-semibold text-slate-700 ml-1">Gumroad URL</label>
                    <input type="url" name="gumroad_url" id="gumroad_url"
                        value="{{ old('gumroad_url', $book->gumroad_url) }}" placeholder="https://gumroad.com/l/your-book"
                        class="w-full bg-slate-50 border border-slate-200 text-slate-800 text-sm rounded-lg focus:ring-2 focus:ring-primary/20 focus:border-primary block p-3 outline-none transition-all">
            ...
            <div class="space-y-1">
                ...
                <div
                    class="border-2 border-dashed border-slate-200 rounded-xl p-8 text-center hover:bg-slate-50 transition-colors cursor-pointer relative group">
                    <input type="file" name="cover_image" id="cover_image" accept="image/*"
                        class="absolute inset-0 w-full h-full opacity-0 cursor-pointer z-10">
                    <div class="space-y-2">
                        <span
                            class="material-symbols-outlined text-[40px] text-slate-300 group-hover:text-primary transition-colors">cloud_upload</span>
            ...
            <div class="flex gap-8 border-t border-slate-100 pt-6">
                ...
                <label class="inline-flex items-center cursor-pointer relative">
                    <input type="checkbox" name="is_published" value="1" class="absolute opacity-0 w-full h-full cursor-pointer peer z-10" {{ old('is_published', $book->is_published) ? 'checked' : '' }}>
                    <div
                ...
                <label class="inline-flex items-center cursor-pointer relative">
                    <input type="checkbox" name="is_free" value="1" class="absolute opacity-0 w-full h-full cursor-pointer peer z-10" {{ old('is_free', $book->is_free) ? 'checked' : '' }}>
                    <div
                        class="relative w-11 h-6 bg-slate-200 peer-focus:outline-none rounded-full peer peer-checked:after:translate-x-full rtl:peer-checked:after:-translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:start-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-green-500">
                    </div>
            ...
            <div class="pt-4 flex gap-4">
                <button type="submit"
                    class="w-full md:w-auto px-8 py-3 bg-primary hover:bg-orange-600 text-white font-bold rounded-lg shadow-lg hover:shadow-orange-500/30 transition-all flex items-center justify-center gap-2">
